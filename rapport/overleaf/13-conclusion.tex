...
\begin{itemize}
  ...
  \item \textbf{Chapitre~4 (Sprints 3, 4 et 5)~:} cœur métier et
    ...
    réelles, modèle Night Panel (production nocturne complémentaire),
    analyse financière sur 25 ans), approche
    ...
    devis avec acceptation client, facturation automatique et envoi de la
    facture PDF par e-mail via workflow \textbf{n8n}~;
  \item \textbf{DevOps (Chapitre~5)~:} industrialisation backend
    (Docker multi-stage, CI/CD GitHub Actions, GHCR, Trivy/Gitleaks,
    observabilité Prometheus/Grafana/Loki), déploiement de démonstration
    \textbf{Vercel + VM Azure}, et base IaC Kubernetes pour évolution
    future~; développement local conservé via Docker Compose et proxy Vite.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{|L{5cm}|C{2cm}|X|}
    ...
    \hline
    Déploiement multi-environnement & \textcolor{green!70!black}{\checkmark} Oui & Vercel + VM Azure \\
    \hline
  \end{tabularx}
\end{table}
