...
function EntrepriseTab({ onSave, userId }: { onSave: () => void; userId: string }) {
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState(() => settingsService.getCompanySettings(userId));

  const handleSave = () => {
    setSaving(true);
    settingsService.saveCompanySettings(userId, form);
    settingsService.saveTimestamp(userId);
    setSaving(false);
    onSave();
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, hqAddress: e.target.value }))}
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <select
              ...
              value={form.currency}
              onChange={(e) => setForm((prev) => ({ ...prev, currency: e.target.value as "TND" | "EUR" | "USD" }))}
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, defaultVat: Number(e.target.value) || 0 }))}
              className={inputCls}
            />
          </div>
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, powerUnit: e.target.value as "kWc" | "Wc" | "MWc" }))}
            >
...
function PreferencesTab({ onSave, userId }: { onSave: () => void; userId: string }) {
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState(() => settingsService.getPreferencesSettings(userId));

  const handleSave = () => {
    setSaving(true);
    settingsService.savePreferencesSettings(userId, form);
    settingsService.saveTimestamp(userId);
    setSaving(false);
    onSave();
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              className={`${inputCls} appearance-none`}
              value={form.language}
              onChange={(e) => setForm((prev) => ({ ...prev, language: e.target.value as "fr" | "en" }))}
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, dateFormat: e.target.value as "DD/MM/YYYY" | "MM/DD/YYYY" | "YYYY-MM-DD" }))}
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, numberFormat: e.target.value as "1 234,56" | "1,234.56" }))}
            >
          ...
          <div className="md:col-span-2">
            ...
            <select
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, dashboardView: e.target.value as "map-projects" | "charts-only" | "kpi-actions" }))}
            >
              <option value="map-projects">Carte + Projets récents</option>
              <option value="charts-only">Graphiques uniquement</option>
              <option value="kpi-actions">KPIs + Actions rapides</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className={card} style={shadow}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="relative">
              <input
                value={form.defaultTilt}
                onChange={(e) => setForm((prev) => ({ ...prev, defaultTilt: Number(e.target.value) || 0 }))}
                type="number"
                className={`${inputCls} pr-10`}
              />
          ...
          <div>
            ...
            <div className="relative">
              <input
                value={form.defaultLoss}
                onChange={(e) => setForm((prev) => ({ ...prev, defaultLoss: Number(e.target.value) || 0 }))}
                type="number"
                className={`${inputCls} pr-10`}
              />
              <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-sm text-gray-400">%</span>
          ...
          <div>
            ...
            <div className="relative">
              <input
                value={form.defaultElectricityPrice}
                onChange={(e) => setForm((prev) => ({ ...prev, defaultElectricityPrice: Number(e.target.value) || 0 }))}
                type="number"
                step="0.001"
          ...
          <div className="flex items-center justify-between md:col-span-1">
            ...
            <Toggle
              on={form.aiRecommendation}
              onChange={() => setForm((prev) => ({ ...prev, aiRecommendation: !prev.aiRecommendation }))}
            />
          </div>
        </div>
      </div>
...
function NotificationsTab({ onSave, userId }: { onSave: () => void; userId: string }) {
  const initial = settingsService.getNotificationSettings(userId);
  ...
  const handleSave = () => {
    settingsService.saveNotificationSettings(userId, { channels, events });
    settingsService.saveTimestamp(userId);
    onSave();
  };
...
function IntegrationsTab({ onSave, userId }: { onSave: () => void; userId: string }) {
  const initial = settingsService.getIntegrationSettings(userId);
  const [provider, setProvider] = useState(initial.aiProvider);
  const [apiKey, setApiKey] = useState(initial.mistralApiKey);
  const [showKey, setShowKey] = useState(false);
  const [pvgisStatus] = useState<"ok" | "fail">("ok");

  const handleSave = () => {
    settingsService.saveIntegrationSettings(userId, {
      aiProvider: provider as "ollama" | "mistral",
...
function SummaryPanel({ userId }: { userId: string }) {
  const lastSavedAt = settingsService.getLastSavedAt(userId);
    ...
    >
      ...
      <div className="space-y-3">
        {[
          ...
          { label: "Dernière sauvegarde", value: lastSavedLabel },
        ].map((r) => (
...
export function SettingsPage() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      ...
      case "entreprise":
        return <EntrepriseTab onSave={showToast} userId={userId} />;
      case "preferences":
        return <PreferencesTab onSave={showToast} userId={userId} />;
      case "notifications":
        return <NotificationsTab onSave={showToast} userId={userId} />;
      case "integrations":
        return <IntegrationsTab onSave={showToast} userId={userId} />;
    }
  };
